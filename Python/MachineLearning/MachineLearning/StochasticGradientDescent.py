...
def feature_normalization(train, test):
    ...
    """
    # TODO
    n_train = np.size(train, 1)
    n_test = np.size(test, 1)
    f_min = np.min(train, 1)
    f_max = np.max(train, 1)

    train_normalized = (train - np.repeat(f_min, n_train, 1)) / (np.repeat(f_max - f_min, n_train, 1))
    test_normalized = (test - np.repeat(f_min, n_test, 1)) / (np.repeat(f_max - f_min, n_test, 1))
